...
const Profile = () => {
  ...
  const loadProfileData = async () => {
    try {
      ...
      const myVideos = allVideos.filter(v => v.author === (data.username || authUser.username || authUser.name));
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      setEditData(prev => ({ ...prev, avatarUrl: updatedUser.profile.avatarUrl }));
    } catch (err) {
  ...
  const DEFAULT_BANNER = "https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?auto=format&fit=crop&q=80&w=1200";
  const DEFAULT_AVATAR = "https://images.unsplash.com/photo-1633332755192-727a05c4013d?auto=format&fit=crop&q=80&w=300";
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A] text-white">
      ...
      <section className="relative h-48 md:h-64 overflow-hidden group border-b border-white/5">
        <img 
          src={isEditing ? (resolveUrl(editData.bannerUrl) || DEFAULT_BANNER) : (resolveUrl(profile?.profile?.bannerUrl) || DEFAULT_BANNER)} 
          alt="Channel Banner" 
          ...
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-500">
            <div className="w-full max-w-xl space-y-4 bg-[#0D1223]/80 border border-white/10 p-6 rounded-[2rem] shadow-2xl backdrop-blur-xl">
              <div className="flex items-center gap-2 mb-2">
                ...
                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-[#00F0FF]">Visual Branding Uplink</label>
              </div>
              <div className="flex flex-col gap-3">
                ...
                <p className="text-[9px] text-gray-500 uppercase tracking-widest leading-loose">
                  Enter a direct image URL (Unsplash, Imgur, etc.) to project your identity across the Nexus.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-end gap-6 -mt-12 md:-mt-16">
          ...
          <div className="relative group cursor-pointer" onClick={handleAvatarClick}>
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-gradient-to-br from-[#00F0FF] to-[#7000FF] p-[3px] shadow-2xl relative z-10">
              <div className="w-full h-full bg-[#0D1223] rounded-full overflow-hidden border-4 border-[#0A0E1A] relative">
                <img 
                  src={resolveUrl(profile?.profile?.avatarUrl) || DEFAULT_AVATAR} 
                ...
                {isEditing && (
                  <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-2 text-center opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <span className="text-[8px] text-gray-400 mt-1">Upload from device</span>
                  </div>
                )}
          ...
          <div className="flex-1 pb-2 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
              ...
              <CheckCircle2 size={18} className="text-[#00F0FF]" fill="currentColor" fillOpacity={0.2} />
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-4 gap-y-2 text-sm text-gray-400 font-medium">
              <span className="text-white font-bold">@nexus_{profile?.username?.toLowerCase()}</span>
              <span className="flex items-center gap-1"><Users size={14} /> 1.2M Operatives</span>
              <span className="flex items-center gap-1"><Video size={14} /> {userVideos.length} Transmissions</span>
            </div>
            ...
            <div className="mt-4 flex flex-wrap justify-center md:justify-start gap-3">
              {!isEditing ? (
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <Save size={14} className="inline mr-2" /> Synchronize Identity
                  </button>
                  <button 
            ...
            {userVideos.length > 0 ? (
              <div className="flex flex-col lg:flex-row gap-8 bg-[#0D1223]/40 border border-white/5 rounded-[2.5rem] p-6 lg:p-8">
                <div className="lg:w-[45%] aspect-video rounded-3xl overflow-hidden relative group cursor-pointer shadow-2xl">
                  <img src={userVideos[0].thumbnail} alt={userVideos[0].title} className="w-full h-full object-cover" />
                ...
                <div className="flex-1 space-y-4">
                  <div className="px-3 py-1 bg-[#00F0FF]/10 border border-[#00F0FF]/20 rounded-full inline-block">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#00F0FF]">Featured Transmission</span>
                  </div>
                  <h2 className="text-2xl font-black uppercase italic tracking-tight italic">{userVideos[0].title}</h2>
                  <p className="text-gray-400 text-sm line-clamp-3 leading-relaxed italic">
                    {userVideos[0].description || "Nexus transmission data pending. Standard operative feed initiated."}
                  </p>
              ...
              <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
                ...
                <h3 className="text-xl font-bold italic uppercase tracking-widest text-gray-500">Your Nexus channel is empty.</h3>
                <p className="text-gray-600 text-sm mt-2">Initialize your first uplink to begin broadcasting.</p>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              {isManagementMode && (
                <div className="bg-red-500/10 border border-red-500/20 px-4 py-1.5 rounded-lg">
                  <span className="text-[10px] font-black text-red-500 uppercase tracking-widest animate-pulse">Destructive Mode Active</span>
                </div>
              )}
            </div>
          ...
          <div className="max-w-4xl grid md:grid-cols-3 gap-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="md:col-span-2 space-y-8">
              <div className="space-y-4">
                <h3 className="text-lg font-black uppercase tracking-widest text-[#00F0FF]">Neural Transmission Bio</h3>
                {isEditing ? (
                  <textarea 
                    value={editData.bio}
                  ...
                  <p className="text-gray-400 leading-relaxed italic">
                    {profile?.profile?.bio || "No biography transmitted. Initialize neural link to bridge data."}
                  </p>
                )}
              </div>
              
              <div className="space-y-4">
                <h3 className="text-lg font-black uppercase tracking-widest text-[#00F0FF]">Direct Comm-Link</h3>
            ...
            <div className="space-y-8 text-center md:text-left">
              <div className="space-y-6">
                <h3 className="text-lg font-black uppercase tracking-widest text-[#00F0FF]">Sector Data</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-center md:justify-start gap-3 text-gray-400">
                    <Calendar size={18} />
                    <span className="text-xs font-bold uppercase transition-colors hover:text-[#00F0FF]">Joined {new Date(profile?.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex items-center justify-center md:justify-start gap-3 text-gray-400">
                    <Users size={18} />
                    <span className="text-xs font-bold uppercase transition-colors hover:text-[#00F0FF]">1,241,562 Operatives</span>
                  </div>
                </div>
              </div>
            </div>
