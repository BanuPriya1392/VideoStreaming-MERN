...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      login({
        name: data.user.username || data.user.name || "PILOT",
        ...
        <div className="w-full max-w-[450px] bg-[#0D1223]/80 backdrop-blur-2xl border border-[#00F0FF]/20 rounded-[2.5rem] p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full bg-gradient-to-r from-[#00F0FF] to-[#7B68EE] text-white font-black py-4 rounded-xl shadow-[0_0_25px_rgba(0,240,255,0.3)] transition-all flex items-center justify-center gap-2 uppercase text-sm tracking-wide ${
                isLoading
