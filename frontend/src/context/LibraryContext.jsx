...
export const LibraryProvider = ({ children }) => {
  ...
  const toggleWatchlist = (video) => {
    ...
    console.log("LibraryContext: Toggling Watchlist for Video ID:", videoId, video.title);
    
    setWatchlist((prev) => {
      const isSaved = prev.some((v) => (v.id || v._id) === videoId);
      if (isSaved) {
        console.log("LibraryContext: Removing from Watchlist");
        return prev.filter((v) => (v.id || v._id) !== videoId);
      } else {
        console.log("LibraryContext: Adding to Watchlist");
        return [video, ...prev];
      }
    });
  };
